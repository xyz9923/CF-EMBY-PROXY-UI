...
11. GitHub Actions 会先执行 `npm ci` 和 `bash ./build-worker.sh`，成功后再调用 Wrangler 部署到 Cloudflare Workers
...
> 方式 C 不依赖 Cloudflare 的 Git 仓库集成，而是由 GitHub Actions 调用 Wrangler CLI 完成部署；如果你更希望把部署权限收敛在 GitHub Secrets 中，这种方式会更合适。
